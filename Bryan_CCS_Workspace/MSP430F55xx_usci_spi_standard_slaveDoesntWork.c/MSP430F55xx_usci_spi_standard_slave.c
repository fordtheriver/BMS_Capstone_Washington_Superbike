...
//Declare a new structure of type SPI_Mode which holes the states of the SPI slave
...
void SPI_Slave_ProcessCMD(uint8_t cmd)
{
    ...
    switch (cmd)
    {
        case (CMD_TYPE_0_SLAVE):                        //Send slave device id (This device's id)
            ...
            UARTprintstring("Fist Byte Sent");
            TXByteCtr--;
            break;
...
void initGPIO()
{
  ...
  P4DIR |= BIT7;                            //Clears LED pin as output
  P4OUT &= ~(BIT7);
...
*/
{
  ...
  /*
  uint8_t uca0_rx_val = 0;                      // Declares variable to hold read integer
  switch(__even_in_range(UCA0IV,4))
  {
    case 0:break;                               // Vector 0 - no interrupt
    case 2:
        uca0_rx_val = UCA0RXBUF;                // Stores byte in rx register in the rx variable
                if (!(SLAVE_CS_IN & SLAVE_CS_PIN))
        {
            switch (SlaveMode)                  // Finite State Machines switches depending on the states stored in SlaveMode
            {
                  case (RX_REG_ADDRESS_MODE):
                      ReceiveRegAddr = uca0_rx_val;
                      SPI_Slave_ProcessCMD(ReceiveRegAddr); //This is a function that executes a command based off what was sent to the SPI slave
                      break;
                  case (RX_DATA_MODE):
                      ReceiveBuffer[ReceiveIndex++] = uca0_rx_val;//Add received value to receive buffer
                      RXByteCtr--;
                      if (RXByteCtr == 0)
                      {
                          //Done Receiving MSG
